...
use "D:\R script and input data\WEAI_final_input_stata\weai_final_18.dta", clear //
...
// In the current version available, there is no module on time poverty - corrupted file in 2011. Integrate after talk with WEAI managers.

// So far all_indicators were defined so 1 identifies adequate. //
// Now we transform indicators so 1 identifies inadequate. //

foreach var in  feelinputdecagr raiprod_any jown_count jrightanyagr credjanydec_any incdec_count groupmember_any speakpublic_any leisuretime {
...
save "D:\R script and input data\WEAI_final_input_stata\weai_ndepr_18.dta", replace
...
gen sample1 = (feelinputdecagr~=. & raiprod_any~=. & jown_count~=. & jrightanyagr~=.& credjanydec_any~=. & incdec_count~=. & groupmember_any~=. & speakpublic_any~=. & leisuretime~=.) //npoor_z105~=.
*#delimit cr
...
*We now create the indicators’ weights.* //npoor_z105
...
*pause
*gen nation=`c' //NOTE: add * at beginning of this line for single-country calculation
...
foreach var in feelinputdecagr raiprod_any jown_count jrightanyagr credjanydec_any incdec_count groupmember_any speakpublic_any leisuretime {
gen `var'_CH_`k'p=(`var'==1 & ch_20p==1)
...
egen pop_shr_before = total(weight/total_b), by(gender)
...
egen sample_r_before = total(temp), by(gender)
label var sample_r_before "Sample Size of each `r' before Sample Reduction"

egen pop_shr_after = total(weight/total_w) if miss==0, by(gender)
label var pop_shr_after "Weighted Population Share of Each `r' after Sample Reduction"
egen sample_r_after = total(temp) if missing==0, by(gender)
...
save "D:\R script and input data\WEAI_final_input_stata\5D_individual_indices_gender_18.dta", replace  // SAVES, FOR EACH COUNTRY, A DATASET WITH INDIVIDUAL DATA. //
...
egen pop_shr = total(weight/total_w) if miss==0, by(gender district)

* collapse
* The following command will "collapse" our individual results according to the subgroup previously defined. 
//pause nation
collapse  ch_* a_* *_CH_`k'p *_raw w_* EAI_* *_miss missing DAI_* pop_shr* sample_r_* sample_lost_ratio [aw=weight],by(gender district)
...
foreach var in feelinputdecagr raiprod_any jown_count jrightanyagr credjanydec_any incdec_count groupmember_any speakpublic_any leisuretime {
gen `var'_cont_`k'_EAI=((`var'_CH_`k'p* w_`var')/ EA_20p)
label var `var'_cont_`k'_EAI "Decomposed Contribution of `var' to the total Empowerment k=`k'"

gen `var'_cont_`k'_DAI=((`var'_CH_`k'p* w_`var')/ M0_20p)
...
gen cont_group_`k'=M0_20p/DAI_20p*pop_shr
label variable cont_group_`k' "Decomposed Contribution"

gen cont_subgroup_DAI_`k'=M0_20p/DAI_20p*pop_shr_after
label variable cont_subgroup_DAI_`k' "Population Subgroup Contribution to DAI"

gen cont_subgroup_EAI_`k'=EA_20p/EAI_20p*pop_shr_after
label variable cont_subgroup_EAI_`k' "Population Subgroup Contribution to EAI"

capture decode gender district, gen(level)
drop gender district

*gen gender=_n
*label define gender_lab 1 "Male" 2 "Female"
*label values gender gender_lab

save "D:\R script and input data\WEAI_final_input_stata\5D_gender_district_disaggregated_18.dta", replace
...
use "D:\R script and input data\WEAI_final_input_stata\weai_ndepr_18.dta", clear
...
*#delimit;
*npoor_z105
*local varlist_5do feelinputdecagr raiprod_any jown_count jrightanyagr credjanydec_any incdec_count groupmember_any speakpublic_any  leisuretime;

gen sample5do=(feelinputdecagr~=. & raiprod_any~=. & jown_count~=. & jrightanyagr~=.& credjanydec_any~=. & incdec_count~=. & groupmember_any~=. & speakpublic_any~=. & leisuretime~=.)
*#delimit cr
...
bys district: sum ci_above
bys district: sum ci_above [aw=weight]
...
bys district: egen women_n=total(female)
bys district: egen women_wt=total(female*weight)
drop female

* Verification
bys district: gen women_i=(sex==2 & M_cen_ci!=. & W_cen_ci!=.)
bys district: egen women_wt2=total(women_i*weight)
bys district: tab women_wt women_wt2, miss
drop women_i women_wt2

** Headcount ratio of inadequate women

gen inadequate=(ci_above==1 & sex==2)
bys district: egen float inadequate_n = total(inadequate)
gen H=inadequate_n/women_n // Considering unweighted sample //
bys district: egen float inadequate_wt = total(inadequate*weight) 
gen H_wt=inadequate_wt/women_wt // Considering weighted sample //


*Verification
bys district: gen inadequate_i=(M_cen_ci<W_cen_ci & sex==2 & M_cen_ci!=. & W_cen_ci!=.)
bys district: egen inadequate_wt2=total(inadequate_i*weight)
bys district: tab inadequate_wt inadequate_wt2, miss
...
bys district: egen float ci_gap_sum = total(ci_gap*weight)
bys district: gen ci_average=ci_gap_sum/inadequate_wt


** Computation of GPI

bys district: gen H_GPI=inadequate_wt/women_wt
bys district: gen P1=H_GPI*ci_average
bys district: gen GPI=1-P1

**************************
*** Summarize results  ***
**************************

bys district: sum H_GPI ci_average P1 GPI 
bys district: count if sex==2
bys district: tab women_n women_wt

save "D:\R script and input data\WEAI_final_input_stata\GPI_district_disaggregated_18.dta", replace
